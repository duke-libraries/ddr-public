...
RSpec.describe "catalog/index", :type => :feature do
  ...
  describe "faceting" do
    ...
    before do
      ...
      allow(I18n).to receive(:t).with('ddr.admin_set.abc') { 'XYZ Admin Set' }
      allow(I18n).to receive(:t).with('ddr.admin_set.def') { 'UVW Admin Set' }
      allow(I18n).to receive(:t).with('ddr.admin_set.ghi') { 'RST Admin Set' }
