module RenderPartialsHelper
  ...
  def render_document_partial(doc, base_name, locals = {})
    ...
    template = cached_view ['show', view_type, base_name, active_fedora_model, format].join('_') do
      find_document_show_template_with_view(view_type, base_name, active_fedora_model, format, locals)
    end
  ...
  #protected
    ...
    def document_partial_path_templates
      ...
      @partial_path_templates ||= ["%{action_name}_%{index_view_type}_%{format}",
                                   "%{action_name}_%{index_view_type}_default",
                                   "%{action_name}_%{format}_%{active_fedora_model}",
                                   "%{action_name}_%{format}",
  ...
  private
    def find_document_show_template_with_view view_type, base_name, active_fedora_model, format, locals
      document_partial_path_templates.each do |str|
        partial = str % { action_name: base_name, active_fedora_model: active_fedora_model, format: format, index_view_type: view_type }
